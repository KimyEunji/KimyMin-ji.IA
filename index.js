...
printWithDelay(
  `${emojic.wave} Bienvenido al cliente de Kim Min-ji.IA. WhatsApp-Bot\n`,
  500
);
...
client.on("qr", (qr) => {
  ...
  printWithDelay(
    `${emojic.hourglass} Esperando código QR para autenticación...\n`,
    500
  ).then(() => {
...
async function buscarRespuestaEnCodigo(pregunta) {
  ...
  } else if (buscarEnBaseDatos(pregunta, wdata)) {
    return buscarEnBaseDatos(pregunta, wdata);
  } else {
    return null;
  }
}
